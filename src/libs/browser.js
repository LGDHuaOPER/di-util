...
export function browserFactory(root) {
  return function(userAgent) {
    // 验证
    if (!_.isObjectLike(root)) {
      throw new TypeError(
        `the param 'root' of function 'browserFactory' should be one 'ObjectLike', but now is ${Object.prototype.toString.call(
          root
        )}`
      );
    }
    ...
    if (!_.isObjectLike(_navigator)) {
      throw new TypeError(
        `the value for 'root.navigator' of function 'browserFactory' should be one 'isObjectLike', but now is ${Object.prototype.toString.call(
          root.navigator
        )}`
      );
    }
    ...
    var u = _.isString(userAgent)
      ? userAgent
      : _.isString(_navigator.userAgent)
      ? _navigator.userAgent
      : '';
    // 区分是 new 还是 fn()
    var _this = new.target ? this : {};
...
export default browserFactory(
  inBrowser ? (typeof self !== 'undefined' ? self : this) : null
);

export const exportRequireContextKeys = ['default'];
